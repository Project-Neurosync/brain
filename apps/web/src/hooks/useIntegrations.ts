...
export function useIntegrationMutations(projectId: string | undefined) {
  ...
  const connectIntegrationMutation = useMutation({
    mutationFn: (data: ConnectIntegrationRequest) => {
      if (!projectId) {
        throw new Error('Project ID is required to connect an integration')
      }
      return integrationApi.connectIntegration(projectId, data)
    },
  ...
  const updateIntegrationMutation = useMutation({
    mutationFn: ({ integrationId, data }: { integrationId: string; data: UpdateIntegrationRequest }) => {
      if (!projectId) {
        throw new Error('Project ID is required to update an integration')
      }
      return integrationApi.updateIntegration(projectId, integrationId, data)
    },
  ...
  const deleteIntegrationMutation = useMutation({
    mutationFn: (integrationId: string) => {
      if (!projectId) {
        throw new Error('Project ID is required to delete an integration')
      }
      return integrationApi.deleteIntegration(projectId, integrationId)
    },
 ...
 * Supports both project-based and projectless OAuth flows
 */
export function useOAuthIntegration(projectId?: string) {
  const [isAuthenticating, setIsAuthenticating] = useState(false)
  const [_, setPendingProjectId] = useLocalStorage<string | null>('pendingProjectId', null)
  ...
  const startOAuth = useCallback(async (type: string) => {
    ...
    try {
      ...
      // Store the project ID if we have one, to retrieve after OAuth completes
      if (projectId) {
        setPendingProjectId(projectId)
        const redirectUrl = `${window.location.origin}/dashboard/integrations/oauth/callback?provider=${type}`
        const response = await integrationApi.startOAuthFlow(projectId, type, redirectUrl)
        auth_url = response.auth_url
      } else {
        // Projectless flow (e.g., during project creation)
        const redirectUrl = `${window.location.origin}/dashboard/integrations/oauth/callback?provider=${type}`
        const response = await integrationApi.startOAuthFlowWithoutProject(type, redirectUrl)
        auth_url = response.auth_url
      }
      ...
      const handleMessage = (event: MessageEvent) => {
        ...
        if (event.data.type === 'oauth-success') {
          ...
          // Return the OAuth result data for further processing
