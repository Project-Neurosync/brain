...
export default function BillingSettingsPage() {
  ...
  useEffect(() => {
    const fetchTokenUsage = async () => {
      ...
      try {
        const usage = await authApi.getTokenUsage(accessToken)
        setTokenUsage(usage)
      } catch (error) {
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between py-2 border-b">
              <div>
                <p className="font-medium">NeuroSync {user?.subscription_tier ? user.subscription_tier.charAt(0).toUpperCase() + user.subscription_tier.slice(1) : 'Starter'}</p>
                <p className="text-sm text-gray-600">December 2024</p>
              </div>
              <div className="text-right">
                <p className="font-medium">{getSubscriptionPrice(user?.subscription_tier || 'starter')}</p>
