...
export default function StatusPage() {
  ...
  // Fetch real service statuses and incidents from the API
  useEffect(() => {
    const fetchStatusData = async () => {
      try {
        ...
        const servicesResponse = await fetch('/api/v1/status/services', {
          method: 'GET',
          headers: {
        ...
        if (!servicesResponse.ok) {
          throw new Error(`Failed to fetch service statuses: ${servicesResponse.statusText}`)
        }
        ...
        // Fetch incidents
        const incidentsResponse = await fetch('/api/v1/status/incidents', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    // Refresh data every 60 seconds for real-time monitoring
    const intervalId = setInterval(fetchStatusData, 60 * 1000)
  ...
  const handleRefresh = async () => {
    try {
      ...
      setError('Failed to refresh status information. Please try again later.')
    } finally {
      setLoading(false)
    }
  }
