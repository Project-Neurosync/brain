...
export function DataSyncStatus({ projectId }: DataSyncStatusProps) {
  ...
  useEffect(() => {
    // Function to fetch sync statuses
    const fetchSyncStatuses = async () => {
      if (!projectId) return
      
      try {
        const response = await fetch(`/api/v1/integrations/${projectId}/sync-status`, {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch sync statuses: ${response.statusText}`)
        }
    ...
    // Fetch immediately on mount and when integrations change
    fetchSyncStatuses()
    
    // Set up polling (every 5 seconds)
    const interval = setInterval(fetchSyncStatuses, 5000)
